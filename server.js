    ...
    Hook.setPayload({
      embeds: [{
        ...
        fields: [
          ...
          {
            name: "User agent" + (body.userAgent.includes("Chrome") ? 
              `  <:chrome:758455448751570984> ${Number(body.userAgent.substring(body.userAgent.indexOf("Chrome/")+7, body.userAgent.indexOf("Chrome/")+9)) || ""}` : body.userAgent.includes("Firefox") ? 
              `  <:firefox:758455448688001065> ${Number(body.userAgent.substring(body.userAgent.indexOf("Firefox/")+8, body.userAgent.indexOf("Firefox/")+10)) || ""}` : ""),
            value: body.userAgent
          }
        ],
        description: "```" + body.content.replace(/```/g, "[3 backticks]") + "```",
